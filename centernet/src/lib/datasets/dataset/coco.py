...
class COCO(data.Dataset):
  ...
  def __init__(self, opt, split):
    ...
    data_dict = parse_data_cfg(opt.data_cfg)
    db_list_path = data_dict[split]

    with open(db_list_path, 'r') as f:
      img_files = f.read().splitlines()
      self.images = list(filter(lambda x: len(x) > 0, img_files))
    
    self.label_files = [x.replace('images', 'labels').
                                replace('.jpeg', '.txt').
                                replace('.jpg', '.txt').
                                replace('.bmp', '.txt').
                                replace('.png', '.txt') for x in self.images]
