...
class BaseTrainer(object):
    ...
    def train(self, epoch, data_loader, optimizer, use_gpu):
        self.model.train()
        ...
        for i, inputs in enumerate(data_loader):
            ...
            inputs, targets = self._parse_data(inputs, use_gpu)
            ...
            # plot progress
            bar.suffix = 'Epoch: [{N_epoch}][{N_batch}/{N_size}] | Time {N_bt:.3f} {N_bta:.3f} | Data {N_dt:.3f} ' \
                         '{N_dta:.3f} | Loss {N_loss:.3f} {N_lossa:.3f} | Prec {N_prec:.2f} {N_preca:.2f}'.format(
    ...
    def _parse_data(self, inputs, use_gpu):
